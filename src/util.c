...
xvsprintf(char *fmt, va_list ap)
{
	...
	while (1) {
		...
		if (nchars > -1 && nchars < size)
			...
			size = nchars + 1;
		/*
		 * c99 says -1 is an error other than truncation, which thus
		 * will not go away with a larger buffer. To support older
		 * system but not making errors fatal (ratpoison will abort
		 * when trying to get too much memory otherwise), try to
		 * increase a bit but not too much:
		 */
